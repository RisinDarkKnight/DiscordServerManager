...
class TwitchCog(commands.Cog):
    ...
    async def _ensure_token(self):
        ...
        if not TWITCH_CLIENT_ID or not TWITCH_CLIENT_SECRET:
            log.warning("Twitch credentials missing from .env")
            return None
        ...
        async with aiohttp.ClientSession() as s:
            async with s.post(url, params=params) as r:
                ...
                log.debug("Obtained Twitch token, expires in %s", data.get("expires_in"))
                return self._token
    ...
    async def check_streams(self):
        ...
        for gid, gcfg in cfg.items():
            ...
            if not guild:
                log.debug("Guild %s not cached locally", gid)
                continue
            twitch_cfg = gcfg.get("twitch", {})
            streamers = twitch_cfg.get("streamers", [])
            channel_id = twitch_cfg.get("notif_channel")
            role_id = twitch_cfg.get("notif_role")
            if not streamers or not channel_id or not role_id:
                continue
            channel = guild.get_channel(channel_id)
            if not channel:
                log.warning("Twitch notif channel %s not found in guild %s", channel_id, gid)
                continue
                ...
                try:
                    ...
                    if stream:
                        ...
                        if meta.get("notified") == sid:
                            log.debug("Already notified guild %s for %s stream id %s", gid, username, sid)
                            continue
                        ...
                        embed = discord.Embed(title=title or f"{user_name} is LIVE!", url=f"https://twitch.tv/{username}",
                                              description=f"Playing **{game}** • {viewers} viewers", color=discord.Color.purple())
                        if thumb:
                        ...
                        msg = f"{user_name} is live, come say hello :D {mention}"
                        try:
                            await channel.send(content=msg, embed=embed, view=view)
                            log.info("Sent Twitch notification for %s in guild %s", username, gid)
                        except discord.Forbidden:
                            log.exception("Forbidden sending twitch message in guild %s channel %s", gid, channel_id)
                        data[gid]["twitch"][username]["notified"] = sid
                        changed = True
                    else:
                        # offline -> reset notified so next stream will trigger again
    ...
    async def removestreamer(self, interaction: discord.Interaction):
        ...
        if not streamers:
            await interaction.response.send_message("No Twitch streamers tracked.", ephemeral=True)
            return
        ...
        class RemoveView(discord.ui.View):
            ...
            async def select_callback(inner_self, select_interaction: discord.Interaction, select):
                ...
                # clear any saved notified state
                data = load_json(DATA_FILE)
