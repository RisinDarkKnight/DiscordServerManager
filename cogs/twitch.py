...
class TwitchCog(commands.Cog):
    """Twitch tracking: /addstreamer, /removestreamer, /setstreamchannel, /setstreamnotifrole
       Polls Twitch Helix and posts embed + Watch button when streamer goes live.
       Deduped via data.json storing last stream id per (guild, streamer).
    """
    ...
    async def _ensure_token(self):
        ...
        if not TWITCH_CLIENT_ID or not TWITCH_CLIENT_SECRET:
            log.warning("Twitch client id/secret missing in env; Twitch features disabled")
            return None
        ...
        async with aiohttp.ClientSession() as s:
            async with s.post(url, params=params) as r:
                if r.status != 200:
                    log.error("Failed to fetch Twitch token: %s", await r.text())
                    return None
                j = await r.json()
                self._token = j.get("access_token")
                self._token_expires = now + int(j.get("expires_in", 3600))
                log.debug("Obtained Twitch token; expires in %s", j.get("expires_in"))
                return self._token
    ...
    async def _fetch_stream(self, username: str):
        ...
        async with self.session.get(url, headers=headers, params=params) as r:
            if r.status != 200:
                log.debug("Twitch API returned %s for %s: %s", r.status, username, await r.text())
                return None
            j = await r.json()
            items = j.get("data", [])
    ...
    async def check_streams(self):
        ...
        for gid, gcfg in cfg.items():
            ...
            if not guild:
                log.debug("Guild %s not cached", gid)
                continue
            ...
            if not channel:
                log.warning("Twitch notif channel %s missing in guild %s", channel_id, gid)
                continue
                ...
                try:
                    ...
                    if stream:
                        ...
                        if meta.get("notified") == sid:
                            # already notified for this stream id
                            continue
                        ...
                        try:
                            ...
                            log.exception("No permission to send Twitch notification in guild %s channel %s", gid, channel_id)

                        data[gid]["twitch"][username]["notified"] = sid
                        changed = True
                    else:
                        # reset notified when offline so next stream triggers
                        if meta.get("notified"):
                            data[gid]["twitch"][username]["notified"] = None
                            changed = True
                except Exception:
                    log.exception("Error while checking streamer %s in guild %s", username, gid)
    ...
    async def removestreamer(self, interaction: discord.Interaction):
        ...
        class RemoveView(discord.ui.View):
            @discord.ui.select(placeholder="Select a streamer to remove", options=options, min_values=1, max_values=1)
            async def select_callback(self, select, select_interaction: discord.Interaction):
                chosen = select.values[0]
                cfg_local = load_json(CONFIG_FILE)
                cfg_local[gid]["twitch"]["streamers"].remove(chosen)
                save_json(CONFIG_FILE, cfg_local)
                # remove saved notified state
                data = load_json(DATA_FILE)
    ...
    @app_commands.command(name="setstreamchannel", description="Set the channel where Twitch notifications will be posted (admin)")
    @app_commands.checks.has_permissions(administrator=True)
    ...
    @app_commands.command(name="setstreamnotifrole", description="Set the role to ping when a Twitch streamer goes live (admin)")
    @app_commands.checks.has_permissions(administrator=True)
